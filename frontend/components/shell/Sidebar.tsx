...
export function Sidebar() {
  ...
  return (
    ...
    >
        ...
        <Link href={homeFor(user.role)} className="flex min-w-0 items-center gap-3">
          ...
          {!collapsed && (
            <div className="min-w-0">
              <p className="truncate font-display text-base font-extrabold tracking-tight text-white">{CAMPAIGN_NAME}</p>
              <p className="truncate text-xs text-gold/90">{CANDIDATE_NAME} · Mombasa 2027</p>
            </div>
          )}
        </Link>
      </div>
        ...
        <div className={cn("relative mx-4 mb-4 rounded-2xl border border-white/[.07] bg-gradient-to-br from-white/[.06] to-white/[.01] backdrop-blur", collapsed ? "p-2" : "p-4")}>
          {collapsed ? (
            <div className="grid place-items-center" title={`County target ${pct(d.overall.percent)}`}>
              <Ring percent={d.overall.percent ?? 0} size={52} stroke={5}><span className="text-xs font-bold text-white">{Math.round(d.overall.percent ?? 0)}%</span></Ring>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              <Ring percent={d.overall.percent ?? 0} size={58} stroke={6}><span className="text-xs font-bold text-white">{Math.round(d.overall.percent ?? 0)}%</span></Ring>
      ...
      {/* Navigation */}
      <nav className={cn("scroll-dark relative flex-1 space-y-5 overflow-y-auto pb-4", collapsed ? "px-3" : "px-4")}>
        {sections.map((section) => (
          <div key={section}>
            {!collapsed && <p className="mb-1.5 px-3 text-xs font-bold tracking-[.18em] text-slate-500 uppercase">{section}</p>}
              ...
              {items.filter((i) => i.section === section).map(({ href, label, icon: Icon }) => {
                ...
                return (
                  <Link key={href} href={href} title={collapsed ? label : undefined}
                    ...
                    {!!badge && (
                      <span className={cn("rounded-full bg-kenya-red px-1.5 text-xs font-bold leading-5 text-white tabular-nums",
      ...
      <div className={cn("relative border-t border-white/[.07] p-3", collapsed && "flex flex-col items-center gap-2")}>
        ...
        <button onClick={toggle} className={cn("mt-2 flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-medium text-slate-500 transition hover:text-white", collapsed && "justify-center")}
