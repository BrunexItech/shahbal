...
def audience_filter(user: User, a: Audience, *, for_calls: bool = False) -> list:
    ...
    # Places add up: "Mvita + Bamburi ward + one polling station" is everyone in any of them.
    places = []
    if a.constituency_ids:
        places.append(Voter.ward_id.in_(select(Ward.id).where(Ward.constituency_id.in_(a.constituency_ids))))
    if a.ward_ids:
        places.append(Voter.ward_id.in_(a.ward_ids))
    if a.station_ids:
        places.append(Voter.station_id.in_(a.station_ids))
    if places:
        conds.append(or_(*places))
    if a.support:
